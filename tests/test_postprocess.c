...
void test_postprocess_defaults(void)
{
    ...
    TEST_ASSERT_FLOAT_WITHIN(1e-5, DEFAULT_VIGNETTE_INTENSITY, pp.vignette.intensity);
...
void test_postprocess_apply_preset(void)
{
    ...
    TEST_ASSERT_FLOAT_WITHIN(1e-5, PRESET_VINTAGE.vignette.intensity, pp.vignette.intensity);
    TEST_ASSERT_FLOAT_WITHIN(1e-5, PRESET_VINTAGE.grain.intensity, pp.grain.intensity);
    TEST_ASSERT_FLOAT_WITHIN(1e-5, PRESET_VINTAGE.exposure.exposure, pp.exposure.exposure);
    TEST_ASSERT_FLOAT_WITHIN(1e-5, PRESET_VINTAGE.chrom_abbr.strength, pp.chrom_abbr.strength);
...
void test_postprocess_resize(void)
{
    ...
    // Validate that resources were recreated (IDs might change or be reused, but basic check is valid)
    // Actually, glGenFramebuffers might return the same ID if the old one was deleted.
    // Instead check that FBO is still complete and valid

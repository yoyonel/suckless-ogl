...
void test_gpu_timer_is_non_blocking(void)
{
    ...
    // This is not super heavy but enough to cause measurable delay if glFinish waits for it
    // compared to almost instant return.
    for (int i = 0; i < 1000; i++) {
        glClearColor((i % 255) / 255.0f, 0.0f, 0.0f, 1.0f);
        glClear(GL_COLOR_BUFFER_BIT);
    }

    // We do NOT call glFinish here explicitly. We want to see if gpu_timer_elapsed_ms calls it.
    ...
    // If glFinish is present, this will block until all 1000 clears are done.
    // If glFinish is removed, this should return almost immediately (issuing query and checking status).
    ...
    // Assertion:
    // If the optimization is working, this call should be very fast (e.g., < 0.5ms).
    // If glFinish is present, it might take > 1ms depending on GPU speed,
    // but 1000 clears should take some time.
    // Let's be conservative and say if it takes > 2ms it's likely blocking.
    // Ideally we'd compare against a baseline, but "non-blocking" means "fast".

...
void setUp(void) {
    ...
    test_window = glfwCreateWindow(TestWidth, TestHeight, "Test Window", NULL, NULL);
    if (!test_window) {
        glfwTerminate();
...
void tearDown(void) {
    ...
    if (test_window) glfwDestroyWindow(test_window);
    glfwTerminate();
}

void test_benchmark_bloom_render(void) {
    PostProcess post_proc = {0};
    int result = postprocess_init(&post_proc, &gpu_profiler_system, TestWidth, TestHeight);

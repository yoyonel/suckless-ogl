...
void test_gpu_timer_start_leak(void)
{
    ...
    /* Currently, the code DOES generate new queries, so we expect this to fail initially */
    ...
    TEST_ASSERT_EQUAL_INT_MESSAGE(2, mock_gl_get_gen_queries_call_count(), "gpu_timer_start leaked query handles by re-generating them!");
}

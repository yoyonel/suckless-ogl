...
## 2026-02-08 - [DoS] Memory Exhaustion via Image Header
...
## 2026-02-08 - [Buffer Overflow] GPU Buffer Overflow in Billboard Rendering
**Vulnerability:** `billboard_group_update` accepted an arbitrary instance count and wrote to the GPU buffer using `glBufferSubData` without checking if the new count exceeded the buffer's initial capacity.
**Learning:** Dynamic GPU data updates must always track the allocated capacity, not just the current count. Assuming the caller always passes a safe count is dangerous.
**Prevention:** Added `capacity` field to `BillboardGroup`. Implemented bounds checking in `billboard_group_update` to clamp the count and log an error if it exceeds capacity.

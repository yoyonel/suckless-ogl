...
| `SphereInstance` | None (Vectors/Matrices) | None | POD struct. Safe to copy (used heavily in `billboard_sorter.c`). |
...
* **`billboard_sorter.c`**: Performs `SphereInstance` copies during sorting. This is entirely safe as `SphereInstance` contains only value types (cglm math types).

...
3.  **Update Reference**: If the PR is visually correct but fails the automated threshold due to the reasons above, update `tests/ref_frame.raw` by capturing a new frame from the current stable build.

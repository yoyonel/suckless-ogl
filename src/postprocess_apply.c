...
void static postfx_bloom(PostProcess* post_processing)
{
	...
	fx_bloom_render(&post_processing->bloom_fx, &post_processing->bloom,
	                &bloom_ctx);
	gl_debug_pop_group();
}

void static postfx_dof(PostProcess* post_processing)
{
	...
	gl_debug_push_group("PostFX_DepthOfField");
	{
		...
		fx_dof_render(&post_processing->dof_fx, &post_processing->dof,
		              &post_processing->bloom_fx, &dof_ctx);
	}
	gl_debug_pop_group();
}

void static postfx_ae_and_debug_histogram(PostProcess* post_processing)
{
	bool ae_active =
	...
	GPU_STAGE_PROFILER(post_processing->gpu_profiler, "Auto Exposure",
	 ...
	 * Debug is on */
	{
		...
		fx_auto_exposure_render(&post_processing->auto_exposure_fx,
	 ...
	 * next frames) */
	int write_idx = (int)((post_processing->readback.frame_count + 1) % 2);

	if (ae_active && !post_processing->readback.exposure_sync[write_idx]) {
...
void static postfx_mb_debug(PostProcess* post_processing)
{
...
void static ubo_full_rebuild(const PostProcess* post_processing)
{
	...
	ubo.screen_texel_size[1] = 1.0F / (float)post_processing->height;
	...
	ubo.dof_anamorphic_ratio = post_processing->dof.anamorphic_ratio;
	...
	ubo.fog_height_falloff = post_processing->fog.height_falloff;
...
void static upload_ubo(PostProcess* post_processing)
{
	...
	if (post_processing->ubo_dirty) {
		ubo_full_rebuild(post_processing);
...
void static postfx_final_composite(PostProcess* post_processing)
{
	...
	if (postprocess_is_enabled(post_processing, POSTFX_BLOOM)) {
		...
		int mip_idx =
		    post_processing->bloom_fx.bloom_step == BLOOM_FINAL
		        ? 0
		        : post_processing->bloom_fx.debug_mip;
		bloom_tex = post_processing->bloom_fx.mips[mip_idx].texture;
	}

	render_utils_bind_texture_safe(GL_TEXTURE0 + POSTPROCESS_TEX_UNIT_BLOOM,
	                               bloom_tex,
	 ...
	 */
	render_utils_reset_texture_units(0, POSTPROCESS_TEX_UNIT_STENCIL + 1,
	                                 post_processing->gpu.dummy_black_tex);
	...
	/* Render LUT Cube Visualization (if enabled) */
	{
		...
		fx_lut_viz_render(&post_processing->lut_viz_fx,
		                  post_processing->lut3d.texture, &lut_viz_ctx);
	}
...
void postprocess_end(PostProcess* post_processing)
{
	...
	/* Générer le bloom */
	postfx_bloom(post_processing);
	/* DoF Blur Pass */
	postfx_dof(post_processing);
	/* Auto Exposure Pass & Debug Histogram */
	postfx_ae_and_debug_histogram(post_processing);
	/* Motion Blur Pre-Pass (Compute) - Also needed for debug modes */
	postfx_mb_debug(post_processing);
	...
	postfx_final_composite(post_processing);

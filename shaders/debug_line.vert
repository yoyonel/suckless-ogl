...
void getProjectedBounds(vec2 axis, float radius, float projScale, out float outMin, out float outMax) {
    ...
    if (d2 <= r2) { outMin = -1.0; outMax = 1.0; return; }
    float L = sqrt(d2 - r2); 
...
void main()
{
	...
	if (u_billboardMode) {
		...
		if (distSq <= r2 * 1.001) {
			...
			worldPos = sphereCenter; // Degenerate world pos, but clip pos covers screen
		} else {
			...
			// Reconstruct WorldPos for consistency (if needed by frag)
			vec3 vertexViewPos;

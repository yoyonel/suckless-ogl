...
void main() {
    ...
    if (enableDoF != 0) {
        ...
        /* Check if it returned a debug visualization (assumed if drastically different,
           but applyDoF returns valid color or debug color. We just assign it.) */
        color = dofColor;
    }
    ...
    /* Note: if color grading is disabled, we might skip WB?
       Original code had separate check for WB but apply_color_grading combined them.
       Let's check original logic:
       Original:
       if (enableColorGrading != 0) applyWhiteBalance(color)
       if (enableColorGrading != 0) apply_color_grading(color)
       So effectively WB is only applied if ColorGrading is ON.
       My new module combines them, so calling apply_color_grading is correct.
    */

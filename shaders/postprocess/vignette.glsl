...
vec3 applyVignette(vec3 color, vec2 uv) {
    if (enableVignette == 0) return color;

    /* Center UVs: (-1 to 1) */
    vec2 uv_centered = uv * 2.0 - 1.0;

    /* Correct Aspect Ratio for distance calculation to keep roundness circular */
    /* Note: We actually often WANT the vignette to follow screen aspect ratio,
       so we might skip aspect correction or make it optional.
       Standard "cinematic" vignettes usually follow the frame aspect.
       Here we don't correct aspect, so it stretches with the screen (Oval on 16:9).
       To make it perfectly circular on any screen, we would multiply x by aspect.
    */

    /*
     * Roundness Logic:
     * roundness = 0.0 -> Rectangular (hard corners if not handled, but here we treat it closer to box)
     * roundness = 1.0 -> Circular (Natural lens)
    ...
    /* Let's use a modified superellipse approximation which is cheaper/smoother */
    ...
    */
     ...
     * Factor = how much logic to KEEP. 1.0 = keep color, 0.0 = black.
     * At center (dist=0), Factor should be 1.0.
     * At edge (dist=1 aprox), Factor should be low (depending on intensity).
     */
    ...
    float factor = 1.0 - smoothstep(1.0 - softness, 1.0 + softness * 0.5, dist);
